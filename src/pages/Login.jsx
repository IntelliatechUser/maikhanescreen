...
const Login = () => {
  ...
  //         await localStorageUtil.setItem("profile", response);
  ...
  const handleLogin = async (values) => {
    try {
      ...
      if (response?.data?.status !== 200) {
        ...
        // Fetch user profile
        const returnObject = await makeApiRequest("/user/profile", "get", { userId });
  
        if (returnObject.status) {
          const profileResponse = returnObject.response;
          
          console.log("Profile response:", profileResponse);
  
          // Set profile and user data
          setProfile(profileResponse.response);
          // setUser({ user: profileResponse?.data?.response, error: null, success: true });
  
          // Store profile in local storage
           localStorageUtil.setItem("profile", profileResponse.response);
