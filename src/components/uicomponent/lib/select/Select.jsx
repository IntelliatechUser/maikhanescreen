    ...
    <>
      <div className={`relative ${sizeClasses} ${className} `}>
        <button
          ...
          disabled={disabled}
          className={`cursor-pointer  text-secondary bg-white border-2 border-yellow-dark focus:shadow-1xl no-underline transition-all duration-200 ease-in-out rounded-lg flex items-center h-[70px] text-poppins text-regular text-[.8vw]  ${variantClasses}  ${baseClass} ${sizeClasses}  ${className}`}
          ...
          >
            {options?.map((item) => (
              <li
                ...
                className="text-input text-poppins text-regular text-[1.36vw] cursor-pointer hover:bg-gray p-2"
              >
                {item.label}
              </li>
            ))}
