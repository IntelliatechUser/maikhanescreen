...
const UnitDetails = ({ onNext }) => {
    ...
    const fetchDocumentDetails = async (countryCode, mobileNumber, actions, setFieldValue) => {
        ...
        try {
            ...
            const returnObject = await makeApiRequest("/otp/generateOtpForUser", "get", {
                countryCode: "+91",
    ...
    const handleSubmitOTP = async (actions, otp, mobileNumber) => {
        let token = localStorageUtil.getItem("token");
            ...
            //     }
            // );
            const returnObject = await makeApiRequest("/otp/validateOtpForUser", "post", {
                mobileNumber: mobileNumber,
                otp: otp
            });
        ...
        const validationSchema = Yup.object({
            ...
            mobileNumber: Yup.string().required("Required").matches(/^[0-9]{10}$/, 'Mobile number must be exactly 10 digits').nullable()
            // .test(
            //   'mobileNumberValidation',
            //   function (value) {
            //     const { contactType } = this.parent;
            //     const isVerified = localStorageUtil.getItem("mobileverify") === 'Mobile is Verified';
            //     if (currentFlow === 'get') {
            //         return true;
            //       }
            //     // If contactType is 'mobile' or 'both' and phone is not verified
            //     if ((contactType === 'mobile' || contactType === 'both') && !isVerified) {
            //       if (!value) {
            //         alert("dd");
            ...
            //         return this.createError({ message: 'Phone must be verified' });
            //       }
            //     }

            //     return true; // No validation needed if phone is verified or for other contact types
            //   }
            // )
                                    ...
                                    <div className="mt-5">
                                        ...
                                        <div className="flex flex-col items-start gap-2 lg:flex-row lg:gap-0 lg:items-center">
                                            ...
                                            <Field
                                                ...
                                                onChange={(e) => {
                                                    setVerify("");
                                                    localStorageUtil.removeItem("mobileverify");// Clear localStorageUtil when the mobile number changes
                                                    setFieldValue("mobileNumber", e.target.value);
                                                }}
                                            // disabled={localStorageUtil.getItem("mobileverify")=="Mobile is Verified" ? true :false}
                                            />
                                            {localStorageUtil.getItem("mobileverify") !== "Mobile is Verified" ? <button type="button" className="ml-2 border border-customOrange text-[#FF9F08] py-2 px-4 rounded-md flex-grow" disabled={diabledForm} onClick={(event) => {
